    ...
    if (mState == NOT_INITIALIZED) {  // 初始化
        ...
        if (!mbOnlyTracking) {  // SLAM模式
            ...
            if (mState == LOST) {
                ...
                // mbVO为false表示匹配了很多的MapPoints，跟踪正常
                // mbVO为true表示此帧匹配了很少的MapPoints(<10)，要跪的节奏
                if (!mbVO) {
                    ...
                    // mbVO为true表示些帧匹配了很少的3D点，少于10个，要跪的节奏，既做跟踪又做定位

                    bool bOKMM = false;         // 通过motion model跟踪的结果
                    bool bOKReloc = false;      // 通过重定位方法跟踪的结果
                    vector<MapPoint*> vpMPsMM;  // 运动模型的地图点
                    vector<bool> vbOutMM;       // 运动模型后发现的外点
                    cv::Mat TcwMM;
                    if (!mVelocity.empty()) {
                        // 运动模型非空，根据运动模型来计算位姿
                    ...
                    if (bOKMM && !bOKReloc) {
                        ...
                        // 只要重定位成功，整个跟踪过程正常进行。定位与跟踪，更相信重定位
                        mbVO = false;
                    }

                    bOK = bOKReloc || bOKMM;  // 有一个成功，就认为执行成功了
                }
            }
        }
        ...
        // 上一步主要是两Reference
        // KF跟踪(跟踪上一帧或参考帧)，这里搜索局部关键帧后搜集所有局部MapPoints，然后在局部MapPoints Reference
        // KF和当前帧进行投影匹配，得到更多匹配的MapPoints后进行Pose优化
        if (!mbOnlyTracking) {
            // 只前面两两帧匹配过程正常，才进行优化
            if (bOK) {
                bOK = TrackLocalMap();
...
void Tracking::UpdateLastFrame() {
    ...
    // 上一帧在世界坐标系下的位姿: T_lw = T_lr * T_rw. l: last, r: reference
    mLastFrame.SetPose(Tlr * pRef->GetPose());

    // 如果上一帧为关键帧, 或者是单目情况, 则退出
...
/**
 * @brief 根据匀速运动模型对上一帧的MapPoint进行跟踪
 *
 * 1. 如果单目，计算上一帧相对于世界坐标系的位姿。双目和RGBD暂未了解
 * 2. 根据运动关系估计当前帧的位姿
 * 3. 将上一帧的MapPoints投影到当前帧的图像平面上, 在投影的位置一定区域内匹配(加快匹配速度)
 * 4. 位姿优化
 * 5. 根据姿态剔除外点(误匹配)
 *
 * @return
...
bool Tracking::TrackWithMotionModel() {
    ...
    // 对于单目, 只计算上一帧相对于世界坐标系的位姿
    UpdateLastFrame();

    // 认为两帧之间的相对运动和之前两帧之间的运动相同, 从而估计当前帧的位姿, T_CW = T_CCl * T_ClW, C为当前帧，Cl为上一帧
    ...
    if (mSensor != System::STEREO)
        ...
        th = 7;
    // 根据匀速模型对上帧的MapPoint进行跟踪，根据上一帧特征点对应的3D点投影位置缩小特征点匹配范围
    ...
    // Discard outliers
    // 剔除位姿优化后的outerlier，和前面TrackReferenceKeyFrame()类似
    int nmatchesMap = 0;
    for (int i = 0; i < mCurrentFrame.keyPointNum; i++) {
        if (mCurrentFrame.mvpMapPoints[i]) {
            if (mCurrentFrame.mvbOutlier[i]) {
    ...
    if (mbOnlyTracking) {
        // 如果在纯定位模式时中追踪的地图点非常少，那么这里的mbVO标记会置位
        mbVO = nmatchesMap < 10;
        return nmatches > 20;
    }
...
bool Tracking::TrackLocalMap() {
    ...
    // 2. 在局部地图中查找与当前帧匹配的MapPoints，在视野范围内的点和当前帧的特征点进行投影匹配
    SearchLocalPoints();
 ...
 *
 * 在局部地图中查找在当前帧视野范围内的点，在视野范围内的点和当前帧的特征点进行投影匹配
 *
 * @return
 */
void Tracking::SearchLocalPoints() {
    // Do not search map points already matched
    // 1. 遍历当前帧的Map Points，标记这些MapPoints不参与之后的搜索，因为当前的MapPoints一定在当前帧的视野中
    ...
    for (vector<MapPoint*>::iterator vit = mvpLocalMapPoints.begin(), vend = mvpLocalMapPoints.end(); vit != vend;
        ...
        // 2.1 判断LocalMapPoints中的点是否在视野内
        if (mCurrentFrame.isInFrustum(pMP, 0.5)) {
            // 观测到该点的帧数+1，该MapPoint在某些帧的视野范围内
            pMP->IncreaseVisible();
    ...
    if (nToMatch > 0) {
        ...
        // 如果不久前进行重定位，那么进行一个更加广泛的搜索，阈值增大
        if (mCurrentFrame.mnId < mnLastRelocFrameId + 2) {
            th = 5;
        }
        // 2.2. 对视野范围内的MapPoints通过投影进行特征点匹配
        matcher.SearchByProjection(mCurrentFrame, mvpLocalMapPoints, th);
    }
}

/**
 * @brief 更新local map, 包括
 * 1. 关键帧K1(与当前帧有相同点的关键帧)，关键帧K2(与K1相邻的关键帧)
 * 2. 由这些关键帧观测到的Map Points
 *
 * @return
 */
...
/**
 * @brief 更新局部Map Points，即局部KF的所有地图点
 *
 * @return
 */
 ...
 * @brief 更新局部关键帧
 *
 * 遍历当前帧的MapPoints，将观测到这些MapPoints的关键帧和相邻的关键帧取出，更新local KFs
 * 怎么定义“相邻关键帧”，从程序中看指的是和他们具有较好的共视关键帧，以及其父子关键帧
 *
 * @return
 */
void Tracking::UpdateLocalKeyFrames() {
    // Each map point vote for the keyframes in which it has been observed
    // 1. 遍历当前帧的MapPoints，记录所有能观测到当前帧MapPoints的关键帧，也就是投票
    ...
    // points
    // 策略1：能观测到当前帧MapPoints的关键帧作为关键帧，K1
    for (map<KeyFrame*, int>::const_iterator it = keyframeCounter.begin(), itEnd = keyframeCounter.end(); it != itEnd;
        ...
        // 更新mnTrackReferenceForFrame防止在策略2中重复添加局部关键帧
        mvpLocalKeyFrames.push_back(it->first);
        pKF->mnTrackReferenceForFrame = mCurrentFrame.mnId;
    }

    // Include also some not-already-included keyframes that are neighbors to already-included keyframes
    // 策略2：与策略1得到的局部关键帧K1共视程度很高的关键帧作为局部关键帧
    for (vector<KeyFrame*>::const_iterator itKF = mvpLocalKeyFrames.begin(), itEndKF = mvpLocalKeyFrames.end();
         itKF != itEndKF; itKF++) {
        ...
        // 策略2.1: 最佳共视的10帧，如果共视帧不足10帧，那么返回所有具有共视关系的关键帧
    ...
    // 更新当前帧的参考关键帧，即与自己共视程度最高的关键帧作为参考关键帧
    // Copy from Others:
    // 我觉得设立参考关键帧的一个目的就是，后面的各种优化过程中只会优化关键帧的位姿，那普通帧的位姿就放弃？nonono，
    // 但是更新吧我们的计算量又承担不过来，so就设立一个参考关键帧，我管你参考关键帧有没有进行优化，只要我（普通帧）保证能够和你参考关键帧的
    // 相对位姿不变就阔以了
    if (pKFmax) {
...
bool Tracking::Relocalization() {
    ...
    int nCandidates = 0;  // 有效的候选关键帧
    // 遍历所有的候选关键帧，确定出满足要求的关键帧并为其创建PnP求解器
    ...
    bool bMatch = false;  // 是否已经找到相匹配的关键帧
    ...
    while (nCandidates > 0 && !bMatch) {
        for (int i = 0; i < nKFs; i++) {
            ...
            if (!Tcw.empty()) {
                ...
                // 成功被两次找到的地图点的集合，其实就是经过RANSAC之后的内点
                set<MapPoint*> sFound;
                ...
                // 通过PoseOptimization进行优化求解，只优化位姿，不优化地图点，返回内点数量
                int nGood = Optimizer::PoseOptimization(&mCurrentFrame);
                // 如果优化后的内点数目不够多，直接跳过本次循环
                ...
                // 如果内点较少，通过投影的方式对之前未匹配的点进行匹配，再进行优化求解
                if (nGood < 50) {
                ...
                // 如果当前关键帧已经有足够的内点了，那么就认为当前关键帧和当前帧匹配上了
                if (nGood >= 50) {
                    bMatch = true;
                    break;
                }
